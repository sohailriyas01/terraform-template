...
# Managed only when enabled, so disabling leaves the bucket unversioned rather than
# trying to set an invalid "Disabled" state on a bucket that was already versioned.
...
resource "aws_s3_bucket_logging" "this" {
  ...
  target_prefix = coalesce(var.logging_target_prefix, "${var.bucket}/")
}
...
data "aws_iam_policy_document" "tls" {
  count = var.enable_tls_enforcement ? 1 : 0
